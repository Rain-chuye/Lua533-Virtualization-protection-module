...
static void free_bytecode(lua_State *L, CYBytecode *bc) {
    if (!bc) return;
    luaM_freearray(L, bc->code, bc->sizecode);
    luaM_freearray(L, bc->k, bc->sizek);
    luaM_free(L, bc);
}
...
static CYBytecode *translate_proto(lua_State *L, Proto *p) {
    ...
    for (int i = 0; i < p->sizek; i++) setobj2n(L, &bc->k[i], &p->k[i]);
...
static int buildEntry(lua_State *L) {
    ...
    LClosure *f = (LClosure *)lua_topointer(L, 1);
    Proto *p = f->p;
...
static int cyluavm_interpreter(lua_State *L) {
    ...
    for (;;) {
        ...
        switch (op) {
            case CYOP_MOVE: setobj2s(L, RA(i), RB(i)); break;
            ...
            case CYOP_SELF: { StkId ra = RA(i); setobj2s(L, ra + 1, RB(i)); luaV_gettable(L, RB(i), RKC(i), ra); break; }
            ...
            case CYOP_CONCAT: { int b = CY_GET_B(i); int c = CY_GET_C(i); L->top = base + c + 1; luaV_concat(L, c - b + 1, c); setobj2s(L, RA(i), base + b); break; }
            ...
            case CYOP_TESTSET: if (l_isfalse(RB(i)) != CY_GET_C(i)) setobj2s(L, RA(i), RB(i)); else pc++; break;
            ...
            case CYOP_FORLOOP: { StkId ra = RA(i); if (ttisinteger(ra)) { lua_Integer step = ivalue(ra+2); lua_Integer idx = ivalue(ra) + step; lua_Integer limit = ivalue(ra+1); if ((step > 0) ? (idx <= limit) : (limit <= idx)) { pc += CY_GET_sBx(i); chgivalue(ra, idx); setobj2s(L, ra+3, ra); } } break; }
            default: break;
        }
    }
}
