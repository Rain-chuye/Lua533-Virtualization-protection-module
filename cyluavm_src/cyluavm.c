...
static int buildEntry(lua_State *L) {
    ...
    LClosure *f = clLvalue(L->top - lua_gettop(L));
    Proto *p = f->p;
...
static int cyluavm_interpreter(lua_State *L) {
    ...
    for (;;) {
        ...
        switch (op) {
            ...
            case CYOP_GETUPVAL: {
                int b = CY_GET_B(i);
                setobj2s(L, RA(i), &cl_func->upvalue[b + 1]);
                break;
            }
            case CYOP_SETUPVAL: {
                int b = CY_GET_B(i);
                setobj(L, &cl_func->upvalue[b + 1], RA(i));
                luaC_barrier(L, cl_func, RA(i));
                break;
            }
            case CYOP_GETTABUP: {
                int b = CY_GET_B(i);
                TValue *upv = &cl_func->upvalue[b + 1];
                TValue *rc = RKC(i);
            ...
            case CYOP_SETTABUP: {
                ...
                TValue *upv = &cl_func->upvalue[a_val + 1];
                TValue *rb = RKB(i);
                TValue *rc = RKC(i);
            ...
            case CYOP_CALL: {
                int b = CY_GET_B(i);
                int nresults = CY_GET_C(i) - 1;
                if (b != 0) L->top = RA(i) + b;
                if (luaD_precall(L, RA(i), nresults)) {
            ...
            case CYOP_TAILCALL: {
                int b = CY_GET_B(i);
                if (b != 0) L->top = RA(i) + b;
                luaD_precall(L, RA(i), LUA_MULTRET);
                return cyluavm_interpreter(L);
            }
            case CYOP_RETURN: {
                int b = CY_GET_B(i);
            ...
            case CYOP_FORLOOP: {
                ...
                if (ttisinteger(ra)) {
                    ...
                    lua_Integer idx = ivalue(ra) + step;
